...
class NeuralNet:
    def __init__(self, size: int):
        ...
        h_conv1 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(256, 3, padding="same")(x_image))
        )
        h_conv2 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(256, 3, padding="same")(h_conv1))
        )
        h_conv3 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(256, 3, padding="same")(h_conv2))
        )
        h_conv4 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(256, 3, padding="valid")(h_conv3))
        )
        h_conv4_flat = Flatten()(h_conv4)

        s_fc1 = Dropout(0.3)(
            Activation("relu")(BatchNormalization(axis=1)(Dense(512)(h_conv4_flat)))
        )
        s_fc2 = Dropout(0.3)(
    ...
    def train(self, examples: List[Tuple[np.ndarray, np.ndarray, np.ndarray]]):
        ...
        self.model.fit(x=inputs, y=[target_pis, target_vs], batch_size=32, epochs=15)
